...
var _ = Describe("Aliases", func() {
	...
	It("can delete an alias from the docs index", func() {
		...
		Expect(cmd.Err()).NotTo(HaveOccurred())
		Expect(client.FTSearch(ctx, "aliasdel", "@command:set", ftsearch.NewQueryOptions()).Err()).To(HaveOccurred())
	})

	It("can move an alias to a different index", func() {
		cmd := client.FTAliasAdd(ctx, "docalias", "customers")
		Expect(cmd.Err()).NotTo((HaveOccurred()))
		search := client.FTSearch(ctx, "docalias", "@command:set", ftsearch.NewQueryOptions())
	...
	It("can find the same result via the original index and the alias", func() {
		...
		Expect(cmd.Err()).NotTo(HaveOccurred())
		searchAlias := client.FTSearch(ctx, "alias2", "@command:set", ftsearch.NewQueryOptions())
		Expect(searchAlias.Err()).NotTo(HaveOccurred())
		searchIndex := client.FTSearch(ctx, "docs", "@command:set", ftsearch.NewQueryOptions())
